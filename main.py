...
def sample_gpu_utilization(samples, stop_event, interval) -> List[str]:
...
def run_inference(prompt, model_name, max_tokens, base_url, request_timeout):
    payload = {
        "model": model_name,
...
def run_benchmark(args):
    ...
    avg_gpu_util = mean(gpu_utilization_samples) if gpu_utilization_samples else 0
...
def parse_args():
    ...
    parser.add_argument("--max-tokens", default=DEFAULT_MAX_TOKENS, help="Max number of completion tokens per prompt.")
    parser.add_argument("--gpu-interval", default=DEFAULT_GPU_SAMPLING_INTERVAL_S, help="Interval in which to sa,ple GPU utilization (seconds).")
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL, help="Base URL for RHEL AI inference server.")
    parser.add_argument("--timeout", default=DEFAULT_REQUEST_TIMEOUT, help="Specify request timeout for prompting.")
